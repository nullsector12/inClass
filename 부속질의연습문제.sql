...
from (select empno, ename, job, sal from emp where job = 'ANALYST') e1, emp e2
where e1.sal > e2.sal and e2.job != 'ANALYST'
order by sal
;
...
from emp
where deptno = (select deptno from emp where ename = 'BLAKE') and ename != 'BALKE'
;
...
from (select distinct deptno from emp where ename like '%K%') e1, emp e2
where e1.deptno = e2.deptno and e2.ename not like '%K%'
;
...
from (select deptno from dept where loc = 'DALLAS') loc, emp e
...
from (select deptno from dept where dname = 'RESEARCH') d, emp e
...
from emp e, (select distinct deptno from emp where ename like '%M%') e2
where sal > (select avg(sal) from emp) and e.deptno = e2.deptno
;
...
from (select deptno from emp where job = 'MANAGER') e1, emp e2
where e2.deptno = e1.deptno and job != 'MANAGER'
order by e2.deptno
;